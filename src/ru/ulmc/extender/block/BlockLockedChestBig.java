...
public class BlockLockedChestBig extends BlockContainer {

	private final Random random = new Random();
	private int chestType;
	
	public BlockLockedChestBig(int i, String name) {		
	...
	public void onBlockAdded(World par1World, int par2, int par3, int par4) {
		...
		} else if (i1 == this.blockID) {
			this.unifyAdjacentChests(par1World, par2, par3, par4 + 1);
		} else if (j1 == this.blockID) {
			this.unifyAdjacentChests(par1World, par2 - 1, par3, par4);
		} else if (k1 == this.blockID) {
			this.unifyAdjacentChests(par1World, par2 + 1, par3, par4);
		}
	}
			...
			if (l != this.blockID && i1 != this.blockID) {
				if (j1 != this.blockID && k1 != this.blockID) {
					...
					if (Block.opaqueCubeLookup[l] && !Block.opaqueCubeLookup[i1]) {
						b0 = 3;
					} else if (Block.opaqueCubeLookup[i1] && !Block.opaqueCubeLookup[l]) {
						b0 = 2;
					} else if (Block.opaqueCubeLookup[j1] && !Block.opaqueCubeLookup[k1]) {
						b0 = 5;
					} else if (Block.opaqueCubeLookup[k1] && !Block.opaqueCubeLookup[j1]) {
						b0 = 4;
					}
				} else {
					...
					if (j2 == 2) {
						b0 = 2;
					} else if ((Block.opaqueCubeLookup[l] || Block.opaqueCubeLookup[l1]) && !Block.opaqueCubeLookup[i1]
							&& !Block.opaqueCubeLookup[i2]) {
						b0 = 3;
					} else if ((Block.opaqueCubeLookup[i1] || Block.opaqueCubeLookup[i2]) && !Block.opaqueCubeLookup[l]
				...
				if (j2 == 4) {
					b0 = 4;
				} else if ((Block.opaqueCubeLookup[j1] || Block.opaqueCubeLookup[l1]) && !Block.opaqueCubeLookup[k1]
						&& !Block.opaqueCubeLookup[i2]) {
					b0 = 5;
				} else if ((Block.opaqueCubeLookup[k1] || Block.opaqueCubeLookup[i2]) && !Block.opaqueCubeLookup[j1]
						&& !Block.opaqueCubeLookup[l1]) {
					b0 = 4;
				}
			}
	...
	public boolean canPlaceBlockAt(World par1World, int par2, int par3, int par4) {
		...
		return l > 1 ? false : (this.isThereANeighborChest(par1World, par2 - 1, par3, par4) ? false : (this
				.isThereANeighborChest(par1World, par2 + 1, par3, par4) ? false : (this.isThereANeighborChest(par1World,
				par2, par3, par4 - 1) ? false : !this.isThereANeighborChest(par1World, par2, par3, par4 + 1))));
	}
	...
	private boolean isThereANeighborChest(World par1World, int par2, int par3, int par4) {
		return par1World.getBlockId(par2, par3, par4) != this.blockID ? false
				: (par1World.getBlockId(par2 - 1, par3, par4) == this.blockID ? true : (par1World.getBlockId(par2 + 1, par3,
						par4) == this.blockID ? true : (par1World.getBlockId(par2, par3, par4 - 1) == this.blockID ? true
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, int par5) {
		...
		TileEntityLockedChest TileEntityLockedChest = (TileEntityLockedChest) par1World.getBlockTileEntity(par2, par3, par4);

		if (TileEntityLockedChest != null) {
			TileEntityLockedChest.updateContainingBlockInfo();
		}
	}
	...
	public void breakBlock(World par1World, int par2, int par3, int par4, int par5, int par6) {
		TileEntityLockedChest lockedChestTE = (TileEntityLockedChest) par1World.getBlockTileEntity(par2, par3, par4);

		if (lockedChestTE != null) {
			for (int j1 = 0; j1 < lockedChestTE.getSizeInventory(); ++j1) {
				ItemStack itemstack = lockedChestTE.getStackInSlot(j1);
				...
				if (itemstack != null) {
					...
					for (float f2 = this.random.nextFloat() * 0.8F + 0.1F; itemstack.stackSize > 0; par1World
						...
						entityitem = new EntityItem(par1World, (double) ((float) par2 + f), (double) ((float) par3 + f1),
	 ...
	 */
	public boolean onBlockActivated(World par1World, int x, int y, int z, EntityPlayer player, int par6,
		...
		if (par1World.isRemote) {
			...
			if(player.username != null && player.username.equals(lockedChestTE.getOwnerName())) {
				isAllowToOpen = true;
			} else if(player.capabilities.isCreativeMode){
				isAllowToOpen = true;
				// later...
			}
			UltimateExtender.logger.info("Username: " + player.username + "; WrittenUsernaeme: " + lockedChestTE.getOwnerName() + "; isAllowToOpen: " + isAllowToOpen); 
			if(isAllowToOpen) {
				IInventory iinventory = this.getInventory(par1World, x, y, z);
	
				if (iinventory != null) {
					player.openGui(UltimateExtender.instance, GuiLockedChest.GUI_ID, par1World, x, y, z);
				}
			} else {
				player.addChatMessage("Access denied!");
			}
			return true;
		}
	}
	...
	public IInventory getInventory(World par1World, int par2, int par3, int par4) {
		Object object = (TileEntityLockedChest) par1World.getBlockTileEntity(par2, par3, par4);
		...
		if (object == null) {
			...
			return null;
				...
				&& (par1World.isBlockSolidOnSide(par2 - 1, par3 + 1, par4, DOWN) || isOcelotBlockingChest(par1World,
						par2 - 1, par3, par4))) {
			return null;
		} else if (par1World.getBlockId(par2 + 1, par3, par4) == this.blockID
				&& (par1World.isBlockSolidOnSide(par2 + 1, par3 + 1, par4, DOWN) || isOcelotBlockingChest(par1World,
			...
			return null;
				...
				&& (par1World.isBlockSolidOnSide(par2, par3 + 1, par4 - 1, DOWN) || isOcelotBlockingChest(par1World, par2,
						par3, par4 - 1))) {
			return null;
		} else if (par1World.getBlockId(par2, par3, par4 + 1) == this.blockID
				&& (par1World.isBlockSolidOnSide(par2, par3 + 1, par4 + 1, DOWN) || isOcelotBlockingChest(par1World, par2,
			...
			if (par1World.getBlockId(par2 - 1, par3, par4) == this.blockID) {
				object = new InventoryLargeChest("container.chestDouble", (TileEntityLockedChest) par1World.getBlockTileEntity(
						par2 - 1, par3, par4), (IInventory) object);
			}

			if (par1World.getBlockId(par2 + 1, par3, par4) == this.blockID) {
				object = new InventoryLargeChest("container.chestDouble", (IInventory) object,
						(TileEntityLockedChest) par1World.getBlockTileEntity(par2 + 1, par3, par4));
			}

			if (par1World.getBlockId(par2, par3, par4 - 1) == this.blockID) {
				object = new InventoryLargeChest("container.chestDouble", (TileEntityLockedChest) par1World.getBlockTileEntity(
						par2, par3, par4 - 1), (IInventory) object);
			}

			if (par1World.getBlockId(par2, par3, par4 + 1) == this.blockID) {
				object = new InventoryLargeChest("container.chestDouble", (IInventory) object,
						(TileEntityLockedChest) par1World.getBlockTileEntity(par2, par3, par4 + 1));
			}
	...
	public int isProvidingWeakPower(IBlockAccess par1IBlockAccess, int par2, int par3, int par4, int par5) {
		if (!this.canProvidePower()) {
			...
			int i1 = ((TileEntityLockedChest) par1IBlockAccess.getBlockTileEntity(par2, par3, par4)).numUsingPlayers;
